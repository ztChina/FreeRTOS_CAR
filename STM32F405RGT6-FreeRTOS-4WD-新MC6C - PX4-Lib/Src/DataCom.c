...
void DataCom_RX_Function(DataCom_RX_Struct *DataCom_RX, uint8_t *Buff)
{
	...
	switch(Buff[3])
	{
		...
		case 5://反馈小车状态
		{
			...
			Usart2_TX_Buf[4]= SystemState;//车辆状态
			Usart2_TX_Buf[5]= 0;
			Usart2_TX_Buf[6]= 0;
			int16_t Temperature = ICM42605.Temperature * 10;
			Usart2_TX_Buf[7]= Temperature >> 8;//温度			
		...
		case 10://超声波
		{
			...
			Usart2_TX_Buf[6]= 0;
			Usart2_TX_Buf[7]= ((int16_t)ICM42605.Temperature) >> 8;//温度			
